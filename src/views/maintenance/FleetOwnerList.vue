        ...
        <div class="dispatch-table">
          <b-card>
            ...
            <div class="newButton">
              <b-button
                variant="primary"
                @click="$router.push('/Maintenance/2017041901085014/new/fleetowner')"
              >
                <i class="fa fa-plus-circle"></i>&nbsp;New Trucker
              </b-button>
            </div>
            <b-row>
            ...
            >
              <template slot="action_button" slot-scope="data">
                <router-link
                  tag="button"
                  :to="'/Maintenance/2017041901085014/modify/fleetowner/' + data.item.fleetownerId"
          ...
          <b-list-group v-for="(item,index) in items" :key="`dynlist-${index}`" class="shadow">
            <b-list-group-item class="flex-column align-items-start">
              <div class="list-field">
                <span class="mb-1 dis">Trucker ID:{{item.fleetownerId}}</span>
              </div>
              ...
              <div class="list-field">
                <span>Contact Person:{{item.contactperson}}</span>
              </div>
              <div class="list-field">
                <span>
                  Status:
              ...
              <div class="row">
                <div class="col">
                  <router-link
                    tag="button"
                    :to="'/Maintenance/2017041901085014/modify/fleetowner/' +item.fleetownerId"
...
export default {
  ...
  methods: {
    ...
    onexport: function() {
      JSONToCSVConvertor(this.items, "Trucker List", true);
    },
    initTable: function() {
      ...
      let promise = api.instance.get(
        "wfms/get-fleet-owner-list?param=&appname=wfms&brancode=" +
      ...
      return promise
        .then(resp => {
          self.items = resp.data.fleetowners;
      ...
      api.instance
        .delete(
          "wfms/remove-fleet-owner?fleetownerId=" +
            this.selectedItem.fleetownerId
        )
        .then(resp => {
          this.initTable();
