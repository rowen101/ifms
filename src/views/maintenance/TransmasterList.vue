        ...
        <div class="dispatch-table">
          <b-card>
            ...
            <div class="newButton">
              <b-button
                variant="primary"
                @click="$router.push('/Maintenance/636859984536802183/new/transmaster/')"
              >
                <i class="fa fa-plus-circle"></i>&nbsp;New Transmaster
              </b-button>
            </div>
            <b-row>
            ...
            >
              <template slot="action_button" slot-scope="data">
                <router-link
                  ...
                  :to="'/Maintenance/636859984536802183/modify/transmaster/' + data.item.id"
                >
          ...
          <b-list-group v-for="(item,index) in items" :key="`dynlist-${index}`" class="shadow">
            <b-list-group-item class="flex-column align-items-start">
              <div class="list-field">
                <span class="mb-1 dis">Transtype: {{item.transtype}}</span>
              </div>

              <div class="list-field">
                <span>Transcode: {{item.transcode}}</span>
              </div>
              <div class="list-field">
                <span>Transdescription: {{item.transdescription}}</span>
              </div>
              <div class="list-field">
                <span>
                  Status:
              ...
              <div class="row">
                <div class="col">
                  <router-link
                    tag="button"
                    :to="'/Maintenance/636859984536802183/modify/transmaster/' +item.id"
...
export default {
  ...
  methods: {
    ...
    onexport: function() {
      JSONToCSVConvertor(this.items, "Transmaster List", true);
    },
    initTable() {
      ...
      let promise = api.instance.get(
        "wfms/get-transmaster-list?param=&brancode=" +
      ...
      return promise
        .then(resp => {
          self.items = resp.data.transmaster;
    ...
    onDeleteItem: function() {
      ...
      api.instance
        .delete("wfms/remove-transmaster?ID=" + this.selectedItem.id)
        .then(resp => {
          this.initTable();
    ...
    onPageSelect(page) {
      ...
      self.$store.commit("setLoading", true);
      let promise = api.instance.get(
        "get-transmaster-list?param=&brancode=" +
          self.$store.getters.selectedWarehouse +
          "&tk=" +
          self.perPage +
          "&pg=" +
