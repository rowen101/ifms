      ...
      <b-col sm="12" class="mt-1">
        ...
        <b-card :header="appendtext + ' Indicator Setup'">
          ...
          <b-form @submit="onSubmit" v-if="!isShowAlert">
            ...
            <!--            <b-form-group label="GroupCode*">-->
            <!--              <v-select v-model.trim="selectgroupcode" :options="optionsgroupcode" label="groupcode"></v-select>-->
            <!--            </b-form-group>-->
            <b-form-group label="ValueDataType*">
              <b-form-select
                id="transtype"
                :plain="true"
                :options="['TEXT','NUMERIC','BOOLEAN']"
  ...
  methods: {
    ...
    onSubmit: function(evt) {
      ...
      if (this.appendtext == "Create") {
        axios
          .post(
            this.$store.getters.apiName + "/api/itsek/add-indicator",
            this.form
          )
          .then(() => {
        ...
        axios
          .put(
            this.$store.getters.apiName + "/api/itsek/edit-indicator",
            this.form
          )
          .then(() => {
            this.isShowAlert = true;
    ...
    GetgroupCode: function() {
      ...
      axios
        .get(
          self.$store.getters.apiName +
            "/api/itsek/get-groupId?department=" +
        ...
        axios
          .get(
            this.$store.getters.apiName +
              "/api/itsek/get-indicator-by-id?indicatorId=" +
